...
function closePhotoModal() {
  ...
  function renderTransportGuide(groups) {
    const container = $('#locationTransport');
    ...
    groups.forEach((group) => {
      ...
      card.className = 'transport-card animate-item';
      card.setAttribute('data-animate', 'fade-up');

      const title = document.createElement('h3');
      title.className = 'transport-card__title';
      title.textContent = group.title || '';
      card.appendChild(title);

      if (group.lead) {
        const lead = document.createElement('p');
        lead.className = 'transport-card__lead';
        lead.textContent = group.lead;
        card.appendChild(lead);
      }

      const list = document.createElement('ul');
      list.className = 'transport-card__list';

      (group.items || []).forEach((text) => {
        const item = document.createElement('li');
        item.className = 'transport-card__item';
  ...
  function initLocation() {
    ...
    renderTransportGuide(CONFIG.transportGuide);
  ...
  function renderAccounts(accounts, containerId) {
    ...
    accounts.forEach((acc) => {
      ...
      detail.append(` ${acc.bank} ${acc.number}`);

      const copyButton = document.createElement('button');
      copyButton.className = 'account-item__copy';
      copyButton.type = 'button';
      copyButton.dataset.account = `${acc.bank} ${acc.number} ${acc.name || ''}`.trim();
  ...
  function initAccounts() {
    ...
    accountSection.addEventListener('click', (e) => {
      const button = e.target.closest('.account-item__copy');
      if (!button || !accountSection.contains(button)) return;
      copyToClipboard(button.dataset.account, '계좌번호가 복사되었습니다');
    });
  }
  ...
  function observeAnimatedElements(root = document) {
    ...
    $$('.animate-item:not(.is-visible)', root).forEach((el) => {
  ...
  async function init() {
    ...
    observeAnimatedElements($('#storyPhotos'));
    observeAnimatedElements($('#galleryGrid'));
  }
